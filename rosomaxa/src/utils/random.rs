...
/// Selects an index from samples using softmax (Boltzmann) distribution with given temperature.
pub fn select_softmax_index(samples: &[Float], temperature: Float, random: &dyn Random) -> usize {
    assert!(!samples.is_empty());

    // If we ever see +inf, treat it as a hard-max tie among +inf arms.
    if samples.iter().any(|&s| s.is_infinite() && s.is_sign_positive()) {
        let idxs = samples
            .iter()
            .enumerate()
            .filter(|(_, s)| s.is_infinite() && s.is_sign_positive())
            .map(|(idx, _)| idx)
            .collect::<Vec<_>>();
        let pick = random.uniform_int(0, (idxs.len() - 1) as i32) as usize;
        return idxs[pick];
    }

    let temperature = temperature.max(f64::EPSILON);
    let max_sample = samples.iter().copied().filter(|s| s.is_finite()).max_by(|a, b| a.total_cmp(b)).unwrap_or(0.0);

    // Stable softmax: w_i = exp((s_i - max_s) / T). Since (s_i - max_s) <= 0,
    // weights are in (0, 1] and never overflow.
    let mut weights = Vec::with_capacity(samples.len());
    let mut sum = 0.0;
    for &s in samples {
        let w = if s.is_finite() { ((s - max_sample) / temperature).exp() } else { 0.0 };
        sum += w;
        weights.push(w);
    }

    if sum <= f64::EPSILON {
        // Degenerate case: fall back to argmax tie-break.
        return random_argmax(samples.iter().copied(), random).unwrap_or(0);
    }

    let mut r = random.uniform_real(0.0, sum);
